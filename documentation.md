...
3. **AI Runtime** – Ollama now runs as its own Docker service, and the deployment flow pulls the configured model inside that container.
...
3. Pulls base images and builds the app images so the stack is ready to start fully inside Docker.
...
2. Starts the `ollama` container and pulls the configured model inside Docker.
...
3. Refreshes the Ollama service and makes sure the configured model exists inside the container.
